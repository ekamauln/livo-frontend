...
export function UserDialog({
  ...
  const fetchUserDetail = useCallback(
    async (id: number) => {
      try {
        setLoading(true);
        const result = await userManagerApi.getUserById(id);
  ...
  const updateProfile = async (data: ProfileFormData) => {
    ...
    try {
      setUpdating(true);
      const result = await userManagerApi.updateUserProfile(userId, {
        email: data.email,
  ...
  const changePassword = async (data: PasswordFormData) => {
    ...
    try {
      setUpdating(true);
      const result = await userManagerApi.updateUserPassword(userId, {
  ...
  const changeStatus = async (isActive: boolean) => {
    ...
    try {
      setUpdating(true);
      const result = await userManagerApi.updateUserStatus(userId, {
        is_active: isActive,
      });
  ...
  const fetchAllRoles = useCallback(async () => {
    try {
      ...
      const result = await userManagerApi.getRoles(1, 1000);
  ...
  const assignRole = async () => {
    ...
    try {
      setUpdating(true);
      const result = await userManagerApi.assignUserRole(userId, {
  ...
  const removeRole = async (roleName: string) => {
    ...
    try {
      setUpdating(true);
      const result = await userManagerApi.removeUserRole(userId, {
        role_name: roleName,
      });

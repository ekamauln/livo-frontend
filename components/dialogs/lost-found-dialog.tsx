...
const lostFoundSchema = z.object({
  product_sku: z.string().min(1, "Lost found SKU is required"),
  reason: z.string().min(1, "Lost found reason is required"),
...
export function LostFoundDialog({
  ...
  const fetchLostFoundDetail = useCallback(
    async (id: number) => {
      try {
        setLoading(true);
        const response = await lostFoundApi.getLostFoundItemById(id);
  ...
  const updateLostFound = async (data: LostFoundFormData) => {
    ...
    try {
      setUpdating(true);
      const response = await lostFoundApi.updateLostFoundItem(lostFoundId, {

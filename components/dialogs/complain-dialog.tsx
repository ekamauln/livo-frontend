...
const editFormSchema = z.object({
  ...
  user_details: z
    ...
    .refine(
      (userDetails) => {
        const operatorIds = userDetails.map((detail) => detail.operator_id);
...
export function ComplainDialog({
  ...
  const isUserAlreadyAdded = (operatorId: number) => {
    const currentUserDetails = editForm.getValues("user_details");
    return currentUserDetails.some(
      (detail) => detail.operator_id === operatorId
    );
  };

  // Handle user selection with duplicate check
  const handleUserSelection = (operatorId: number, index: number) => {
    if (isUserAlreadyAdded(operatorId)) {
    ...
    // Update the form
    editForm.setValue(`user_details.${index}.operator_id`, operatorId);
    if (selectedUser) {
      editForm.setValue(
  ...
  const fetchUsers = useCallback(async () => {
    try {
      ...
      const result = await userManagerApi.getUsers(1, 100); // Get first 100 users
    ...
    if (complain) {
      editForm.reset({
        ...
        user_details: (complain.user_details || []).map((detail) => ({
          ...
          fee_charge: detail.fee_charge,
          user_name:
            detail.operator?.full_name || `User ID: ${detail.operator_id}`,
        })),
      });
    }
  }, [complain, editForm]);
                        ...
                        <div className="border rounded-md">
                          <Table>
                            <TableBody>
                              ...
                              <TableRow>
                                <TableCell className="w-32">Store</TableCell>
                                <TableCell className="w-10">:</TableCell>
                                <TableCell>
                                  {complain.store?.name || "-"}
                                </TableCell>
                              ...
                              <TableRow>
                                <TableCell className="w-32">Solution</TableCell>
                                <TableCell className="w-10">:</TableCell>
                                <TableCell className="wrap-break-word whitespace-normal max-w-md">
                                  {complain.solution || "-"}
                                </TableCell>
                                ...
                                <Button
                                  ...
                                  onClick={() => {
                                    if (users.length > 0) {
                                      ...
                                      const usedOperatorIds =
                                        currentUserDetails.map(
                                          (detail) => detail.operator_id
                                        );
                                      const availableUser = users.find(
                                        (user) =>
                                          !usedOperatorIds.includes(user.id)
                                      );

                                      if (availableUser) {
                                        const newIndex = fields.length;
                                        append({
                                          operator_id: availableUser.id,
                                          fee_charge: 0,
                              ...
                              <div className="space-y-3">
                                {fields.map((field, index) => (
                                  ...
                                  >
                                    <FormField
                                      control={editForm.control}
                                      name={`user_details.${index}.operator_id`}

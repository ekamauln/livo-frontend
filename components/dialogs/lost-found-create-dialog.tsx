...
export function LostFoundCreateDialog({
  ...
  const onSubmit = async (data: LostFoundCreateFormData) => {
    setIsLoading(true);
    try {
      const response = await lostFoundApi.createLostFoundItem(data);
      const lostFound = response.data as LostFound;
      onLostFoundCreated(lostFound);
      toast.success("Lost found item created successfully");
      lostFoundCreateForm.reset();
      onOpenChange(false);
    } catch {
      toast.error("Failed to create lost found item. Please try again.");
    } finally {
      setIsLoading(false);
    }
  };
      ...
      <DialogContent className="min-w-[700px] max-w-4xl max-h-[90vh] overflow-y-auto">
        <DialogHeader>
          <DialogTitle className="flex items-center gap-2">
            <PackagePlus className="h-5 w-5" /> Create New Lost Found Item
          </DialogTitle>
          <DialogDescription>
            Create a new lost found item by filling out the form below.
          </DialogDescription>
          <Separator

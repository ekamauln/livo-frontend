...
// Schema for QC online details
const qcOnlineDetailSchema = z.object({
  box_id: z.number().min(1, "Box is required"),
  quantity: z.number().min(1, "Quantity must be at least 1"),
});

type ProductVerificationData = z.infer<typeof productVerificationSchema>;
type QcOnlineDetailData = z.infer<typeof qcOnlineDetailSchema>;
...
export function QcOnlineForm({
  ...
  const [qcOnlineDetails, setQcOnlineDetails] = useState<QcOnlineDetailData[]>(
    []
  );
  ...
  // Form for QC online detail
  const detailForm = useForm<QcOnlineDetailData>({
    resolver: zodResolver(qcOnlineDetailSchema),
    defaultValues: {
      box_id: 0,
      quantity: 1,
    },
  ...
  // Add QC online detail
  const handleAddDetail = (data: QcOnlineDetailData) => {
    // Check if box already added
    if (qcOnlineDetails.some((d) => d.box_id === data.box_id)) {
    ...
    setQcOnlineDetails([...qcOnlineDetails, data]);
  ...
  // Remove QC online detail
  const handleRemoveDetail = (index: number) => {
    setQcOnlineDetails(qcOnlineDetails.filter((_, i) => i !== index));
    toast.info("Detail removed");
  };

  // Submit QC online
  const handleSubmit = async () => {
    if (qcOnlineDetails.length === 0) {
    ...
    try {
      ...
      await qcOnlineApi.createQcOnline({
        tracking: order.tracking,
        details: qcOnlineDetails,
      });
        ...
        <div className="space-y-6">
          ...
          <Card>
            <CardContent>
              ...
              {/* QC Online Details List */}
              {qcOnlineDetails.length > 0 && (
                <div className="mt-4">
                  <Separator className="my-4" />
                  <p className="text-sm font-medium mb-2">
                    Added Box ({qcOnlineDetails.length})
                  </p>
                  <div className="grid grid-cols-5 gap-3">
                    {qcOnlineDetails.map((detail, index) => (
                      <div
                        key={index}
                        className="flex items-center justify-between p-2 border rounded-lg"
                      >
          ...
          <div className="flex gap-2 justify-end">
            ...
            <Button
              ...
              disabled={qcOnlineDetails.length === 0 || loading}
            >

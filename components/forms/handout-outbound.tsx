...
export default function HandoutOutbound() {
    ...
    try {
      ...
      if (filterData.length > 0) {
        autoTable(doc, {
          ...
          didParseCell: function (data) {
            // Add more height to the receiver row (last row)
            if (data.row.index === filterData.length - 1) {
              data.cell.styles.minCellHeight = 15; // Increase height for receiver row
            }
          },
        });
      }
      ...
      autoTable(doc, {
        ...
        body: reportData.map((item) => [
          ...
          `${item.outbound_operator.full_name}`,
        ]),
  ...
  const handleExportToExcel = async () => {
    try {
      ...
      infoData.push(["Generated", format(new Date(), "dd MMM yyyy - HH:mm")]);
      infoData.push(["Receiver", ""]);

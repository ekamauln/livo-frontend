...
export default function HandoutComplain() {
  ...
  const handleGenerateAndPrintReport = async () => {
    try {
      ...
      const totalProducts = reportData.reduce((total, item) => {
        return (
          total +
          item.product_details.reduce((itemTotal, detail) => {
      ...
      reportData.forEach((item) => {
        item.product_details.forEach((detail) => {
          tableData.push([
            tableData.length + 1,
            item.order_ginee_id, // order_ginee_id
            `${detail.product.name}\nSKU: ${detail.product.sku}`, // product name with SKU
  ...
  const handleExportToExcel = async () => {
    try {
      ...
      link.href = url;
      link.download = `Handout_Complain_Report_${format(
        new Date(),
        "yyyyMMdd_HHmmss"
      )}.xlsx`;
      link.click();
